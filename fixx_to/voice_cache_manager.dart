...
class VoiceCacheManager extends GetxService {
  ...
  // Download progress tracking (messageId → progress 0-100)
  ...
  Future<VoiceCacheManager> init() async {
    ...
    print('[VoiceCacheManager] ✅ Initialized with ${_cacheMetadata.length} cached files');
    return this;
  }
  ...
  Future<void> queueDownload({
    ...
    print('[VoiceCacheManager] 📥 Queued download: $messageId (priority: ${priority.name})');
  }

  /// Pre-fetch multiple voice messages (for chat screen)
  Future<void> prefetchVoiceMessages(List<VoicePrefetchRequest> requests) async {
    ...
    print('[VoiceCacheManager] 🚀 Pre-fetching ${requests.length} voice messages');
  }
  ...
  bool isDownloading(String messageId) {
    return downloadStatus[messageId] == VoiceDownloadStatus.downloading;
  }
  ...
  Future<void> _processDownloadQueue() async {
    ...
    if (_downloadQueue.isEmpty || _activeDownloads.length >= MAX_CONCURRENT_DOWNLOADS) {
      return;
    }
  ...
  Future<void> _downloadVoiceMessage(VoiceDownloadTask task) async {
    ...
    try {
      print('[VoiceCacheManager] ⬇️ Downloading: $messageId (attempt ${task.attempts + 1})');
      ...
      await _dio.download(
        ...
        onReceiveProgress: (received, total) {
          if (total > 0) {
            ...
            // print('[VoiceCacheManager] 📊 Progress: ${(progress * 100).toStringAsFixed(1)}%');
          }
        },
      );
      ...
      print('[VoiceCacheManager] ✅ Downloaded: $messageId (${fileSize ~/ 1024}KB)');
  ...
  Future<void> _enforceStorageLimits() async {
    ...
    if (cacheSizeMB > MAX_CACHE_SIZE_MB || _cacheMetadata.length > MAX_CACHED_FILES) {
      print('[VoiceCacheManager] 🧹 Cache limit exceeded (${cacheSizeMB.toStringAsFixed(1)}MB / ${_cacheMetadata.length} files)');
      await _evictLRU();
    }
  }
  ...
  Future<void> _evictLRU() async {
    ...
    print('[VoiceCacheManager] 🗑️ Evicting $toRemove oldest cache entries (LRU)...');
    ...
    try {
      ...
      if (data != null) {
        data.forEach((key, value) {
          _cacheMetadata[key] = VoiceCacheEntry.fromJson(Map<String, dynamic>.from(value));
        });
        print('[VoiceCacheManager] 📥 Loaded ${_cacheMetadata.length} cache entries');
      }
    } catch (e) {
  ...
  Future<void> _saveCacheMetadata() async {
    try {
      final data = _cacheMetadata.map((key, value) => MapEntry(key, value.toJson()));
  ...
  Future<void> _waitForDownload(String messageId, {Duration timeout = const Duration(minutes: 2)}) async {
...
class VoiceCacheEntry {
  ...
  factory VoiceCacheEntry.fromJson(Map<String, dynamic> json) => VoiceCacheEntry(
