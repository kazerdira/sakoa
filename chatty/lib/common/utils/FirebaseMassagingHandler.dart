...
class FirebaseMassagingHandler {
  ...
  static Future<void> _receiveNotification(RemoteMessage message) async {
    if (message.data != null && message.data["call_type"] != null) {
      ...
      } else if (message.data["call_type"] == "contact_request") {
        // Someone sent you a contact request
        var data = message.data;
        var from_name = data["name"] ?? "Someone";
        var from_token = data["token"] ?? "";

        print("[Firebase] 🔔 Contact request notification from: $from_name");
      ...
      } else if (message.data["call_type"] == "contact_accepted") {
        // Someone accepted your contact request
        var data = message.data;
        var from_name = data["name"] ?? "Someone";
        var from_token = data["token"] ?? "";

        print("[Firebase] 🔔 Contact accepted notification from: $from_name");
