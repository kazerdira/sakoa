...
class _MessageVisibilityDetectorState extends State<MessageVisibilityDetector>
  ...
  void _checkVisibility() {
    ...
    final visibleBottom = (position.dy + size.height).clamp(0.0, viewportHeight);
    final visibleHeight = visibleBottom - visibleTop;
    final visibilityRatio = visibleHeight / size.height;

    final isCurrentlyVisible = visibilityRatio >= _visibilityThreshold && 
                                _appInForeground;
  ...
  void _markMessageAsRead() {
    // Import your MessageDeliveryServiceV2 and call markAsRead
    try {
      // Get.find<MessageDeliveryServiceV2>().markAsRead(
      //   chatDocId: widget.chatDocId,
      //   messageId: widget.messageId,
      //   isLastMessage: false, // Determine this based on your logic
      // );
      
      print('[VisibilityDetector] ✅ Marked message as read: ${widget.messageId}');
    } catch (e) {
...
/// 🔥 ENHANCED CHAT LIST WITH VISIBILITY TRACKING
/// Use this widget to wrap your chat ListView
class VisibilityTrackingChatList extends StatelessWidget {
  final List<Widget> children;
  final ScrollController? controller;

  const VisibilityTrackingChatList({
    Key? key,
